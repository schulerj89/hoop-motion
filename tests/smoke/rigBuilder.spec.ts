...
test("auto-builds and exports a rig profile", async ({ page }) => {
  ...
  await page.waitForFunction(() => window.__HOOPMOTION_RIG_READY === true);
  ...
  const artifactDir = path.resolve("docs/artifacts/v1.0.0");
  ...
  await rigDownload.saveAs(path.join(artifactDir, "hoopbot-auto-rig.json"));
});
